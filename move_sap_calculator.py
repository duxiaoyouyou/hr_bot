...
class calculator:
    def __init__(self, shares_to_be_vested, vesting_date_exchange_rate, vesting_date_stock_price,
        ...
        self.withold_income_tax_amount_on_sale_euro = 0.0
        self.withold_income_tax_amount_on_sale_rmb = 0.0
    ...
    def __calculate_withhold_income_tax_amount_on_sale(self):
        self.withold_income_tax_amount_on_sale_euro = self.withold_shares_amount_to_sell * \
                                                 self.vesting_date_stock_price
        self.withold_income_tax_amount_on_sale_rmb = self.withold_income_tax_amount_on_sale_euro * \
                                                     self.vesting_date_exchange_rate
    ...
    def to_calculation_details(self):
        return f'1.	在德国，SAP公司给员工发放了{self.shares_to_be_vested}股SAP股票。发放的同时，需要对这些股票的价值进行税务处理。\n' \
               ...
               f'当前案例：卖掉的股票总价值是：{self.withold_shares_amount_to_sell}股 * {self.vesting_date_stock_price}欧元/股 = {self.withold_income_tax_amount_on_sale_euro}欧元（预估{self.withold_income_tax_amount_on_sale_rmb}人民币）\n' \
               f'4.	在中国，实际的个人税率是20%，这部分税由SAP公司代缴，员工在收到股票收入后可以抵扣。\n' \
               f'当前案例：实际需要交的税是：{self.shares_to_be_vested}股 * {self.vesting_date_stock_price}欧元/股 * {self.vesting_date_exchange_rate} * {self.actual_income_tax_rate*100} = {self.actual_income_tax_amount}人民币，由SAP公司代缴。\n' \
